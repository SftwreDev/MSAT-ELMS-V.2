...
from .forms import StudentInterestsForm,StudentSignUpForm, TakeQuizForm, BaseAnswerInlineFormSet, QuestionForm, TeacherSignUpForm
from .models import Quiz, Student, TakenQuiz, User, Answer, Question, Year_Level
...
@method_decorator([login_required, student_required], name='dispatch')
class StudentsQuizListView(ListView):
    model = Quiz
    ordering = ('name', )
    ...
    def get_queryset(self):
        ...
        queryset = Quiz.objects.filter(year_level__in=student_year_level) \
...
class YearLevelView(CreateView):
    model = Year_Level
    fields = ('name', )
    template_name = 'classroom/teachers/year_level_form.html'

    def form_valid(self, form):
        year_level = form.save(commit=False)
        year_level.owner = self.request.user
        year_level.save()
        messages.success(self.request, "Student's year level successfully created")
        return redirect('exams:create-new-exams')
